...
for (dataset_name in names(datasets)) {
  ...
  for (var in variables) {
    ...
    dataset_results[[var]] <- list(predictions = results, difference = difference_result)
  }
...
predictions_combined <- do.call(rbind, lapply(all_results, function(ds) do.call(rbind, lapply(ds, function(x) x$predictions))))
differences_combined <- do.call(rbind, lapply(all_results, function(ds) do.call(rbind, lapply(ds, function(x) x$difference))))

# Display the results
print(predictions_combined)
print(differences_combined)
```
...
**Using SD values from EPL_Totals (EPL_5) dataset**
...
**Using SD values from respective datasets**
